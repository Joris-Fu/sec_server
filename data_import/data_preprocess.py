...
class graph1_DataPreProcess(object):
    ...
    def flow_preprocess(self):
        ...
        for line in lines:
            ...
            edges.append({'type': 'access', 'INDEX': index(src_ip, dst_ip), 'srcV': src_ip, 'dstV': dst_ip, 'srcPort': src_port, 'dstPort': dst_port, 'time': format_time, 'packages': pkg_num, 'bytes': bytes, 'transProto': protocol, 'tcpFlag': TCP_flag, 'hash':hash_val})
    ...
    def jsp_preprocess(self):
        ...
        for line in lines:
            ...
            #Add edges
            hash_val = hashlib.md5(''.join(line)).hexdigest()
            edges.append({'type':'attack', 'INDEX': index(src_ip, dst_ip), 'srcV': src_ip, 'dstV': dst_ip, 'srcPort': src_port, 'dstPort': dst_port, 'time': format_time, 'in_out_port': in_out_port, 'event': evt_name, 'device': device_name, 'returnValue': return_val, 'hash':hash_val})
            edges.append({'type':'ip2domain', 'INDEX': index(dst_ip, domain), 'srcV': dst_ip, 'dstV': domain, 'hash': hashlib.md5(dst_ip+domain).hexdigest()})
            edges.append({'type':'domain2url', 'INDEX': index(domain, url), 'srcV': domain, 'dstV': url, 'time':format_time, 'hash':hashlib.md5(domain+url).hexdigest()})
            edges.append({'type':'ip2url', 'INDEX': index(src_ip, url), 'srcV': src_ip, 'dstV': url, 'time':format_time, 'hash':hashlib.md5(src_ip+url).hexdigest()})
...
class graph2_DataPreProcess(object):
    ...
    def depmt_preprocess(self):
        ...
        for line in lines:
            ...
            for i in range(ip_end - ip_start + 1):
                ...
                edges.append({'type':'ip2dep', 'INDEX': index(ip_tmp, department), 'srcV': ip_tmp, 'dstV': department, 'hash': hash_val})
    ...
    def download_preprocess(self):
        ...
        for line in lines:
            ...
            edges.append({'type': 'ip2file', 'INDEX': index(dep_ip, file_md5), 'srcV': dep_ip, 'dstV': file_md5, 'time': format_time,'DSI': control_ip, 'DSU': url, 'hash': hash_val})
            edges.append({'type': 'url2file', 'INDEX': index(url, file_md5), 'srcV': url, 'dstV': file_md5, 'hash': hashlib.md5(url+file_md5).hexdigest()})
            edges.append({'type': 'ip2url', 'INDEX': index(control_ip, url), 'srcV': control_ip, 'dstV': url, 'hash': hashlib.md5(control_ip+url).hexdigest()})
    ...
    def dns_preprocess(self):
        ...
        for line in lines:
            ...
            edges.append({'type': 'DNS', 'INDEX': index(src_ip, req_domain), 'srcV': src_ip, 'dstV': req_domain, 'DNSServer': dns_server_ip, 'transProto': protocol, 'reqDomain': req_domain, 'reqType': req_type,'reqCount': req_counts, 'direct': direction, 'respFlag': resp_flag, 'respVal': resp_val, 'RPReqType': rr_req_type, 'RSIP': resolution_ip, 'TTL': ttl, 'time': format_time, 'SR': sampling_rate, 'hash': hash_val})
            edges.append({'type': 'ip2domain', 'INDEX': index(resolution_ip, req_domain), 'srcV': resolution_ip, 'dstV': req_domain})
    ...
    def flow_preprocess(self):
        ...
        for line in lines:
            ...
            edges.append({'type': 'access', 'INDEX': index(src_ip, dst_ip), 'srcV': src_ip, 'dstV': dst_ip, 'srcPort': src_port, 'dstPort': dst_port, 'packages': pkg_num, 'bytes': bytes,'time': format_time, 'tcpFlag': TCP_flag, 'transProto': protocol, 'hash': hash_val})
