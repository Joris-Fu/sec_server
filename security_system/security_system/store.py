...
for line in lines :
    ...
    conn.execute("insert into knowledgeBase_instance (name,ins_type,ip,city,country,lat,lon,port,timestamp) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (ip + "c", "Camera", ip, City, Country, Lat, Lot, port, TimeList[index].strftime("%Y-%m-%d %H:%M:%S") ) )
    index += 1
f_in.close()
"""
conn = sqlite3.connect('db.sqlite3')
