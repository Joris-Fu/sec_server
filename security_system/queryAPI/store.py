...
for line in lines :
    ...
    #try:
    conn.execute("insert into knowledgeBase_instance (name,ins_type,ip,city,country,lat,lon,port,timestamp) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (ip + "_" + port, "Camera" + str(random.randint(0,100), ip, City, Country, Lat, Lot, port, TimeList[index].strftime("%Y-%m-%d %H:%M:%S") ) )
 #   except :
  #      print(ip)
   #     exit(0)
    index += 1
