...
def getLine(request,flag='3months'):
    ...
    while(start!=end):
        ...
        row = cur.fetchone()
        rows[start.strftime('%Y%m%d')] = row[0]*3
...
def getSegPaper(request,itemOnePage,no):
    ...
    result = []
    result.append(int(itemsum))  #閹鍙℃い鐢告桨閺?
    ...
    cur.execute("select title,url,abstract,author,source,tag from paper order by flag DESC" + lim)
    results = cur.fetchall()
    for row in results:
        paper = {}
        paper["title"] = row[0]
...
def getIps(request):
    conn,cur = createConnect()
    result = []
    cur.execute("select * from ips order by totals desc limit 3000")
    results = cur.fetchall()
    for row in results:
        cves = {}
        cves["ip"] = row[0]
...
def getSegIps(request,itemOnePage,no):
    ...
    if no <= 0:
        ...
        return HttpResponse(json.dumps(result))
    lim = " limit "+str(itemOnePage*(no-1))+","+str(itemOnePage)
    cur.execute("select * from ips order by totals desc" + lim)
    results = cur.fetchall()
    for row in results:
        cves = {}
        cves["ip"] = row[0]
...
def VulIndex():
    ...
    for i in range(0,100):
        ...
        for j in range(1,101):
            ...
            loopday = newdate["day"]
        #index[date]=100-tindex
        index[date]=100-tindex-random.randint(0,20)
        newdate = getDate(year, month, day)
        year = newdate["year"]
        month = newdate["month"]
        day = newdate["day"]
...
def getStatistics(request):
    ...
    blog_news_list = ['blogs','cnblogs','icssnews']
    i=0
    while i<3:
        cur.execute("select count(*) from "+ blog_news_list[i]+';')
        row = cur.fetchone()
...
def getPageNum(request,kind,num):
    ...
    #kind=blogs英文博客/kind=cnblogs中文博客/kind=icssnews英文新闻
    results = cur.fetchall()
    for row in results:
        pagenum = {}
