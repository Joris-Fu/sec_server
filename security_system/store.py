...
for line in lines :
    ...
    cmd = "insert into knowledgeBase_instance (name,ins_type,ip,city,country,lat,lon,port,timestamp) values('%s','%s','%s','%s','%s','%s','%s','%s','%s')" % (ip + "_" + port + "_" + str(random.randint(0,100)), "Camera", ip, City, Country, Lat, Lot, port, TimeList[index].strftime("%Y-%m-%d %H:%M:%S") ) 
    try :
        conn.execute(cmd)
        su_number += 1
        conn.commit()
