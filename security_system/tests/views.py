...
def register2(request):
    ...
    if request.method == "POST":
        ...
        if len(filterResult) > 0:#username already exists
            ...
            #
            user = MyUser.objects.create_user(username = username, email = email, password = password,gender = gender, contact = contact, unit = unit, authLevel = 1 )
            user.is_active = True#?????
            user.save()
            #
            r["success"] = 1
...
def addUser(request):
    user = MyUser.objects.create_user(username = "aaaaaa",
                                    email = "123@123.com",
                                    password = "password",
                                    gender = 0, contact = "12345678910", unit = "buaa", authLevel = 1 #1 is the lowest
                                    )
    user.is_active = True#
    user.save()
